...
// ---------------- MongoDB setup (moved OUTSIDE routes) ----------------
const client = new MongoClient(
  `mongodb+srv://${process.env.USERNAME_DB}:${process.env.PASSWORD_DB}@cluster0.kbxs8tk.mongodb.net/?appName=Cluster0`,
);
...
async function connectToMongoDB() {
  ...
  connectingPromise = (async () => {
    try {
      ...
      console.error("MongoDB Connection Error:", err);
      // reset so next request can retry instead of staying stuck
...
// Ensure DB is connected before any route below runs.
// This fixes the Vercel cold-start race condition (404s on /parcels, /users, /role etc).
...
// ---------------- helper functions that use collections ----------------
